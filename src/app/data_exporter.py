"""CSV export and evaluation utilities for classification results."""
...
class EvaluationTracker:
    """Evaluate classification results using labeled CSV rows.

    Expected CSV columns:
        - category: predicted class
        - ground_truth: manually confirmed or corrected true class

    Rows with empty ground_truth are ignored because they are not labeled.
    """

    PREDICTED_COLUMN = "category"
    ACTUAL_COLUMN = "ground_truth"

    def __init__(self) -> None:
        self._labeled_rows: list[dict[str, str]] = []

    def load_labeled(self, csv_path: str) -> None:
        """Read a CSV file and keep only rows where ground_truth is not empty."""
        path = Path(csv_path)
        ...
        required_columns = {self.PREDICTED_COLUMN, self.ACTUAL_COLUMN}
        ...
        labeled_df = df[
            df[self.ACTUAL_COLUMN].astype(str).str.strip() != ""
        ].copy()

        self._labeled_rows = [
            {
                "predicted": str(row[self.PREDICTED_COLUMN]).strip(),
                "actual": str(row[self.ACTUAL_COLUMN]).strip(),
            }
            for _, row in labeled_df.iterrows()
        ]

    def confusion_matrix(self) -> dict[str, dict[str, int]]:
        """Return confusion matrix as {predicted: {actual: count}}."""
        ...
        return {
            predicted: dict(actual_counts)
            for predicted, actual_counts in sorted(matrix.items())
        }

    def accuracy(self) -> float:
        """Return classification accuracy over labeled rows."""
        total = len(self._labeled_rows)
    ...
    def top_errors(self, limit: int = 5) -> list[tuple[str, str, int]]:
        """Return most common mistakes as (predicted, actual, count)."""
        errors = Counter(
        ...
        return [
            ...
            for (predicted, actual), count in errors.most_common(limit)
        ]

    def labeled_count(self) -> int:
        """Return number of labeled rows loaded for evaluation."""
        return len(self._labeled_rows)

    def correct_count(self) -> int:
        """Return number of correctly classified labeled rows."""
        return sum(
            1
            for row in self._labeled_rows
            if row["predicted"] == row["actual"]
    ...
    def print_report(self) -> None:
        """Print accuracy, confusion matrix and top-5 most common errors."""
        total = self.labeled_count()
        correct = self.correct_count()
        accuracy = self.accuracy()

        print("Smart Storage — Evaluation Report")
        print("=" * 50)
        ...
        if total == 0:
            ...
            print("Fill the ground_truth column in output/results.csv, then rerun:")
            print("  uv run python -m src.evaluation --csv output/results.csv")
            return
        ...
        for predicted, actual_counts in matrix.items():
            ...
            print(f"Predicted {predicted}: {actual_summary}")

        print("\nTop-5 errors")
        print("-" * 50)

        errors = self.top_errors(limit=5)

        if not errors:
            print("No errors found.")
            return

        for index, (predicted, actual, count) in enumerate(errors, start=1):
            print(
                f"{index}. predicted={predicted} | "
                f"actual={actual} | count={count}"
            )